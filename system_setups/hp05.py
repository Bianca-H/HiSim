"""HP05 household system setup (HP04 hybrid: undersized HP + oil peak boiler).

HP05 is based on `hp04` (advanced HP + buffer + DHW + cooling + PV + battery + EV + strict comfort recovery),
and changes the space-heating generation to a hybrid system:
- Heat pump is sized to 80% of the ideal lookup capacity
- A conventional oil boiler covers the remaining / peak load
"""
...
from hisim.components import hydraulic_temperature_massflow_mixer
...
def setup_function(my_sim: Any, my_simulation_parameters: Optional[SimulationParameters] = None) -> None:  # noqa: too-many-statements
    """Set up HP05 (HP04 hybrid: undersized HP + oil peak boiler)."""
    ...
    if sizing_mode == "IDEAL_LOOKUP":
        ...
        chosen_hp = heating_system_selection.pick_heat_pump_closest_to_ideal(
            ideal_power_in_watt=hp_target_power_in_watt
        )
        ...
        log.information(
            ...
            f"HP target {hp_target_power_in_watt / 1e3:.2f} kW ({hp_share_of_ideal:.0%}), "
    ...
    # =============================================================================================================================
    # Oil boiler (peak / backup) sized to remaining capacity
    if sizing_mode == "IDEAL_LOOKUP":
        oil_peak_power_w = max(float(ideal_heating_power_in_watt) - float(p_th_set), 0.0)
    else:
        # fallback: cover a meaningful share of building peak if no lookup used
        oil_peak_power_w = 0.2 * float(my_building_information.max_thermal_building_demand_in_watt)
    oil_peak_power_w = max(oil_peak_power_w, 500.0)
    cli_overrides.set_used_value("OIL_BOILER_PEAK_POWER_W", str(int(round(oil_peak_power_w))))

    my_oil_boiler_config = generic_boiler.GenericBoilerConfig.get_scaled_conventional_oil_boiler_config(
        heating_load_of_building_in_watt=oil_peak_power_w,
        building_name="BUI1",
    )
    my_oil_boiler_config.name = "OilBoilerPeak"
    ...
    my_oil_boiler_controller_config.name = "OilBoilerControllerPeak"
    # For peak backup we want it to react quickly to buffer drops.
    my_oil_boiler_controller_config.hysteresis_water_temperature_offset = 2.0
    my_oil_boiler_controller = generic_boiler.GenericBoilerController(
        my_simulation_parameters=my_simulation_parameters,
        config=my_oil_boiler_controller_config,
    )

    # Mix HP + boiler hydraulics before buffer inlet
    my_hydraulic_mixer = hydraulic_temperature_massflow_mixer.HydraulicTemperatureMassflowMixer(
        my_simulation_parameters=my_simulation_parameters,
        config=hydraulic_temperature_massflow_mixer.HydraulicTemperatureMassflowMixerConfig.get_default_config(
            building_name="BUI1",
            name="HeatGenHydraulicMixer",
        ),
    )
    ...
    # Oil boiler peak controller: Weather + buffer + HDS controller (for target flow temperature)
    my_oil_boiler_controller.connect_only_predefined_connections(
        my_weather, my_hot_water_storage, my_heat_distribution_controller
    )
    ...
    my_hot_water_storage.connect_input(
        ...
        my_hydraulic_mixer.component_name,
        my_hydraulic_mixer.MixedTemperature,
    )
    my_hot_water_storage.connect_input(
        my_hot_water_storage.WaterMassFlowRateFromHeatGenerator,
        my_hydraulic_mixer.component_name,
        my_hydraulic_mixer.MixedMassFlow,
    )

    # Connect mixer inputs: heat pump + oil boiler
    my_hydraulic_mixer.connect_input(
        my_hydraulic_mixer.Temperature1,
        my_heatpump.component_name,
        my_heatpump.TemperatureOutputSH,
    )
    my_hydraulic_mixer.connect_input(
        my_hydraulic_mixer.MassFlow1,
        my_heatpump.component_name,
        my_heatpump.MassFlowOutputSH,
    )
    my_hydraulic_mixer.connect_input(
        my_hydraulic_mixer.Temperature2,
        my_oil_boiler.component_name,
        my_oil_boiler.WaterOutputTemperatureSh,
    )
    my_hydraulic_mixer.connect_input(
        my_hydraulic_mixer.MassFlow2,
        my_oil_boiler.component_name,
