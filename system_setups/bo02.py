"""BO02 household system setup (oil boiler + DHW + buffer tank + split AC cooling, no PV).
...
def setup_function(my_sim: Any, my_simulation_parameters: Optional[SimulationParameters] = None) -> None:  # noqa: too-many-statements
    ...
    # Avoid unrealistically high annual cooling energy in mild climates:
    # the controller previously enforced a hard minimum of 10% modulation when on.
    ...
    # Comfort-band cooling request: cap split-AC at a fraction of the same discrete HP nominal as HP variants
    # (IDEAL_LOOKUP + pick_heat_pump_closest_to_ideal), not full catalogue/nominal.
    ...
    log.information(
        ...
        f"{ac_sizing_power_w / 1e3:.2f} kW, aligned with HP02 cooling KPI scale)."
    )
