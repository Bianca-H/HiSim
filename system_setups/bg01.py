"""BG01 household system setup (gas boiler + DHW + buffer tank, no cooling, no PV).

This is based on `bo01`, but replaces the oil boiler with a condensing gas boiler
...
from hisim.components import gas_meter
...
def setup_function(my_sim: Any, my_simulation_parameters: Optional[SimulationParameters] = None) -> None:  # noqa: too-many-statements
    ...
    - Condensing gas boiler for space heating + DHW
    ...
    # Prevent any cooling demand in BG01 (no cooling system in this variant).
    ...
    # Gas boiler sizing (ideal lookup)
    ...
    my_oil_boiler_config = generic_boiler.GenericBoilerConfig.get_scaled_condensing_gas_boiler_config(
        heating_load_of_building_in_watt=boiler_power_w,
        building_name="BUI1",
    )
    my_oil_boiler_config.name = "GasBoiler"
    # Be explicit: ensure OPEX/CO2 use gas factors (kWh-based).
    my_oil_boiler_config.energy_carrier = loadtypes.LoadTypes.GAS
    my_oil_boiler = generic_boiler.GenericBoiler(
        my_simulation_parameters=my_simulation_parameters,
        config=my_oil_boiler_config,
    )

    # Gas meter for operational cost/emission postprocessing aggregation
    my_gas_meter_config = gas_meter.GasMeterConfig.get_gas_meter_default_config(
        building_name="BUI1",
        gas_loadtype=loadtypes.LoadTypes.GAS,
    )
    ...
    my_oil_boiler_controller_config.name = "GasBoilerController"
    ...
    # so no cooling mode is considered in BG01.
